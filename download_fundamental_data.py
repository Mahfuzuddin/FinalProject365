...
def download_fundamental_data(input_file_name, output_file_name):
    ...
    df = pd.DataFrame(
        list(zip(symbol, sector, epsNext5Y, marketCap, totalAssets, totalDebt, freeCashFlow, beta, peRatio)),
        columns=['Symbol', 'Sector', 'EPS Next 5Y', 'Market Cap', 'Total Assets', 'Total Debt', 'Free Cash Flow',
                 'Beta', 'P/E Ratio'])
